  ...
  command.action(
    async (options: {
      ...
      await withCliContext(async ({ prisma }) => {
        ...
        if (jsonMode !== "off") {
          printJsonPayload(bets, jsonMode);
          return;
        }
...
function printBetLine(bet: StoredBet) {
  ...
  const totalCost = formatCurrency(bet.totalCostCents);
  const budget = formatCurrency(bet.budgetCents);
  const leftover = formatCurrency(bet.payload.leftoverCents);
  const seed = bet.payload.seed;
  const mixSummary = summarizeTicketBreakdown(
    bet.payload.ticketCostBreakdown ?? [],
  );

  console.log(
    `[${createdAt}] ${bet.strategyName} · gasto ${totalCost} (budget ${budget}, leftover ${leftover}) · seed ${seed}${mixSummary}`,
  );
}
