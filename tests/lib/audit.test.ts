...
describe('audit', () => {
  ...
  it('retention soft-delete is idempotent (deleted_at only set once)', async () => {
    process.env.VITEST = '1';
    vi.resetModules();
    ...
    const deletedAtIso = '2025-12-17T00:00:00.000Z';

    const first = db
      .prepare('UPDATE audit_logs SET deleted_at = ? WHERE deleted_at IS NULL AND timestamp < ?')
      .run(deletedAtIso, cutoffIso) as unknown as { changes?: number };
    ...
    const rows = db
      .prepare('SELECT * FROM audit_logs WHERE deleted_at IS NOT NULL ORDER BY timestamp DESC LIMIT ?')
      .all(1) as Array<Record<string, unknown>>;

    expect(rows).toHaveLength(1);
    expect(rows[0].deleted_at).toBe(deletedAtIso);
  });
});
