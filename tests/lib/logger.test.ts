...
describe('logger', () => {
  ...
  it('redacts nested secret-like metadata keys and handles circular values', async () => {
    process.env.NODE_ENV = 'production';
    const logger = await importFreshLogger();
    ...
    logger.info('logger.test_nested_redaction', {
      request: {
        headers: {
          authorization: 'Bearer should-not-appear',
          safe: 'ok',
    ...
    const entry = JSON.parse(String(payload)) as {
      metadata?: {
        request?: { headers?: Record<string, unknown>; cookies?: unknown };
        circular?: { self?: unknown };
      };
    };

    expect(entry.metadata?.request?.headers?.authorization).toBe('[REDACTED]');
    expect(entry.metadata?.request?.headers?.safe).toBe('ok');
    expect(entry.metadata?.request?.cookies).toBe('[REDACTED]');
    expect(entry.metadata?.circular?.self).toBe('[CIRCULAR]');
  });
