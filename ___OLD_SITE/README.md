...
- Lotes de apostas são persistidos em `BetBatch` (agregados) e `Bet` (tickets individuais com custo/metadata); consulte `src/services/bet-store.ts` para leitura e `persistBatch` para gravação.
