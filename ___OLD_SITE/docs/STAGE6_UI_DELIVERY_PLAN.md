...
- Links de documentação (`REPO_BASE`) apontam para `seu-usuario/...`, quebrando acesso direto aos arquivos no GitHub remoto `mneves75/megasena-analyser-nextjs`.
...
- [x] Corrigir `REPO_BASE` nas páginas `stats` e `docs` para apontar para `https://github.com/mneves75/megasena-analyser-nextjs`.
