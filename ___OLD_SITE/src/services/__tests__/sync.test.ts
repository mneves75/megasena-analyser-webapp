...
describe("resolveProcessingRange", () => {
  it("processa todo intervalo quando não há limite", () => {
    const range = resolveProcessingRange({
      startConcurso: 101,
      latestConcurso: 110,
    });
    expect(range).toEqual({ totalToProcess: 10, endConcurso: 110 });
  });

  it("trunca pelo limite informado", () => {
    const range = resolveProcessingRange({
      startConcurso: 101,
      latestConcurso: 110,
      limit: 3,
    });
    expect(range).toEqual({ totalToProcess: 3, endConcurso: 103 });
  });

  it("retorna zero quando início é maior que último", () => {
    const range = resolveProcessingRange({
      startConcurso: 120,
      latestConcurso: 110,
      limit: 5,
    });
    expect(range).toEqual({ totalToProcess: 0, endConcurso: 119 });
  });
});
