...
export default async function BetsPage({ searchParams }: BetsPageProps) {
  ...
  const gridRows = bets.map((bet) => {
    const metadata = bet.metadata as Partial<StrategyMetadata> | null;
    const score =
    ...
    return {
      ...
      batchSeed: bet.batch.seed,
      ticketSeed: bet.ticketSeed,
      metadata: metadata ?? null,
      score,
      window: bet.batch.payload.config?.window ?? null,
      batchAverageTicketCostCents: bet.batch.averageTicketCostCents,
    };
  });
