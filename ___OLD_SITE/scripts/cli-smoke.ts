...
const LEGACY_FLAG = ["--dry", "-run"].join("");

if (
  COMMANDS.some((command) =>
    command.args.some((arg) => arg === LEGACY_FLAG),
  )
) {
  throw new Error(
    `[cli-smoke] Legacy flag '${LEGACY_FLAG}' detectado nas definições do smoke. Atualize para '--persist'.`,
  );
}
...
async function ensureDatabaseReady() {
  try {
    let summary = (await captureCliJson({
      title: "summary",
      args: ["summary", "--json"],
    })) as SummaryPayload;
    ...
    if (!Number.isFinite(totalDraws) || totalDraws === 0) {
      console.log(
        "[cli-smoke] Banco sem concursos. Executando sync limitado para preparar dados...",
      );
      await runCli({ title: "sync seed", args: ["sync", "--limit", "5", "--json"] });
      summary = (await captureCliJson({
        title: "summary",
        args: ["summary", "--json"],
      })) as SummaryPayload;
      if (Number(summary?.totalDraws ?? 0) === 0) {
        throw new Error(
          "Sincronização automática falhou em obter concursos. Verifique credenciais ou tente manualmente.",
        );
      }
    }
  } catch (error) {
    ...
    child.on("exit", (code) => {
      ...
      try {
        const candidate = raw
          ...
          .at(-1);
        if (!candidate) {
          throw new Error(`Não foi possível capturar JSON do comando ${command.title}`);
        }
