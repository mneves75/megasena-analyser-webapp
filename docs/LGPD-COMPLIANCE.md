...
- Em ambientes de teste/local-prod, o opt-in `IP_HASH_SECRET_AUTOGENERATE=true` gera um segredo efêmero por processo (logado como `security.ip_hash_secret_autogenerated`). Não é usado em deploy real.
...
- **Provisionamento do segredo:**
  ...
  - `bun run start` (systemd / PM2 / shell) é **sempre fail-closed**: o wrapper `scripts/start-prod.ts` não injeta nenhum opt-in. O operador precisa exportar `IP_HASH_SECRET` antes de subir o serviço, ou habilitar explicitamente `IP_HASH_SECRET_AUTOGENERATE=true` (não recomendado em deploy real).
  - E2E (`bun run test:e2e`): o `playwright.config.ts` define `IP_HASH_SECRET_AUTOGENERATE=true` apenas dentro do `webServer.env`, escopo limitado à execução de testes.
  - O opt-in `IP_HASH_SECRET_AUTOGENERATE=true` é a **única** forma de relaxar a regra. Deployments reais não devem habilitá-lo.
