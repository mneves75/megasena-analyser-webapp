...
ENTREGÁVEIS (exija tudo)
	1.	Design System
	•	Design tokens (cores, tipografia, radii, sombras, espaçamentos) em JSON + tabela.
	•	Biblioteca de componentes: Button, Input, Select, Card, Modal/Sheet, Tabs, Table, Tooltip, Toast, Navbar/Sidebar, Empty States.
	2.	Especificação de Interações
	•	Tabela de estados (default/hover/press/focus/disabled/loading).
	•	Animações (duração 120–220ms, curvas padrão); ripple configurável.
	3.	Fluxos & Páginas
	•	Wire → Hi-fi para: <páginas-chave> com variantes mobile/desktop.
	•	Hierarquia visual e exemplos de casos vazios/erro/loading.
	4.	Cópias (pt-BR)
...
Agora gere a solução completa com base nos INPUTS.

Design the site / app with a premium, world-class UI. 

# Style direction:

- Clean, minimal, Apple/Linear/Notion.so/Mercury-level polish
