...
Neste projeto, o layout raiz lê o nonce de CSP vindo do `proxy.ts`. Isso torna as páginas protegidas por nonce renderizadas por request. A troca é deliberada: CSP estática/SRI preserva cache estático, mas no runtime atual do App Router os scripts inline de streaming ainda são necessários para hidratação completa.
