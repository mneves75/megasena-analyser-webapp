...
Chamadas server-side (páginas SSR e a server action do gerador) repassam à API, byte a byte, os headers de IP de cliente da requisição da página (`lib/api/forwarded-client-ip.ts`). Isso não cria confiança nova — o rewrite `/api/*` já repassa os headers do navegador sem alteração, e a API aplica as mesmas regras de `TRUST_PROXY_HEADERS`/peer confiável aos dois caminhos —, mas, com `TRUST_PROXY_HEADERS=true` (contrato de produção), faz cada chamada contar no bucket do visitante em vez de todos dividirem o bucket de `127.0.0.1`. Esse helper roda no processo Next: se `TRUSTED_CLIENT_IP_HEADER` fixar um header, defina-o também ali, não só na API Bun.

`INTERNAL_API_SECRET` (mínimo de 32 caracteres) é aceito somente quando o peer é loopback e o segredo bate em comparação constante; headers enviados por clientes públicos não são suficientes. Uma chamada interna válida só fica isenta do rate limit quando **não** traz nenhum header de IP de cliente. O critério é a presença do header, não o valor resolvido: um `CF-Connecting-IP: 127.0.0.1` forjado, ou um valor inválido que cai para o peer loopback, não compra a isenção. Quando traz, a chamada é cobrada do visitante — do contrário a server action de geração de apostas seria um caminho sem limite até `/api/generate-bets`. Com `TRUST_PROXY_HEADERS` desligado nenhum cliente é identificável em caminho algum, e a isenção permanece.
