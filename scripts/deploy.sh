...
################################################################################
# Script de Deploy Automatizado - Mega-Sena Analyser
#
# Deploy para VPS Hostinger com isolamento de aplicação
# Usa PM2 + Nginx em ambiente compartilhado
#
...
echo -e "${YELLOW}  Porta: ${APP_PORT}${NC}"
echo -e "${YELLOW}════════════════════════════════════════════════════════════${NC}"
echo ""
...
if ssh -o ConnectTimeout=10 -o BatchMode=yes ${SSH_CONNECTION} "echo 2>&1" > /dev/null 2>&1; then
    print_success "Conexão SSH OK (usando chave)"
else
    print_warning "Conexão requer senha. Por favor, insira quando solicitado."
fi
...
if command -v bun &> /dev/null; then
    echo "📦 Usando Bun para instalar dependências..."
...
if [ "\$RESPONSE" -eq "200" ]; then
    ...
    echo "⚠️  Aplicação retornou HTTP \$RESPONSE"
fi
ENDSSH
...
echo ""
echo "  Ver status:"
echo "    ssh ${SSH_CONNECTION} 'pm2 status'"
echo ""
echo "  Ver logs em tempo real:"
echo "    ssh ${SSH_CONNECTION} 'pm2 logs ${APP_NAME}'"
echo ""
echo "  Reiniciar aplicação:"
echo "    ssh ${SSH_CONNECTION} 'pm2 restart ${APP_NAME}'"
echo ""
echo "  Atualizar dados do banco:"
echo "    ssh ${SSH_CONNECTION} 'cd ${REMOTE_PATH} && bun run db:pull -- --limit 50'"
echo ""
echo "⚠️  Próximos passos:"
echo "  1. Configure o Nginx conforme DEPLOY.md"
echo "  2. Configure SSL com Let's Encrypt (certbot)"
echo "  3. Aponte seu domínio para ${SSH_HOST}"
echo ""
echo -e "${YELLOW}📖 Consulte DEPLOY.md para configuração completa do Nginx${NC}"
echo ""
